...
- **Registration Pipeline:** Students register by submitting Personal details (Name, DOB, Address, Govt ID, Photo) and Academic details (Department, Roll No).
...
- **Login:** Secure login using the generated Library Card No and password.

### 1.2 Dashboard & Profile Management
- **Profile Updates:** Students can edit their personal details. Changes require management approval before taking effect across the system.
...
- **Borrowing Constraints (Business Logic):**
  ...
  - **The Hybrid Fine/Renewal System:** 
...
- **Notifications Inbox:** Students receive automated system notifications and direct messages from Management regarding approvals or restocks.
- **Feedback & Reviews:** Students can submit feedback, condition reports, or general reviews to the Management team.

### 1.4 Digital Catalogue
- **Guest Access:** Non-authenticated users can browse the public catalogue.
- **Search & Filters:** Real-time search by Title or Author, with advanced filtering by Book Category. Designed to handle a vast inventory.
...
- **Requests System:** Students can request the library to order a completely new book or request additional copies of a currently out-of-stock book.
...
- **Login:** Employees log in using pre-assigned Employee IDs (No public registration; accounts provisioned by upper management).
- **Concurrency Control:** The backend utilizes atomic database operations (Optimistic Concurrency Control) to ensure multiple employees can work simultaneously without causing data race conditions (e.g., preventing two admins from approving the same student at the exact same millisecond).
...
- **Pending Approvals Pipeline:**
  - **New Students:** Review Govt IDs and details. Accept or Reject with a mandatory reason/message.
  - **Profile Edits:** Review requested changes from existing students. Accept or Reject with a message.

### 2.3 Inventory & Order Management
- **Order Pipeline:** Interface to order new books or request more copies of existing ones. Tracks the name of the book and the number of copies ordered.
...
### 2.4 Feedback Loop
- **Review Center:** Read incoming student feedback/reviews and reply directly. Replies are routed to the student's Notification Inbox.
